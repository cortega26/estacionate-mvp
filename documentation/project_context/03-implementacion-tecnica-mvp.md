...
> technical context only. Any payment, PSP, refund, payout, or parking
> monetization language below describes historical/demo ideas or future blocked
> phases, not enabled Phase 1 production behavior.
...
2. ¿Qué problemas enfrentan con estacionamientos para invitados?
3. ¿Estarían dispuestos a compartir 20% de ingresos por digitalización?
4. ¿Cómo valida actualmente la conserjería las visitas?
5. ¿Qué resistencias esperan de residentes por cobro de estacionamiento?
```
...
- Ocupación promedio: 15%+ de capacidad disponible
...
-- Bloques de disponibilidad (inventario dinámico)
...
│             │           │             │           │             │
│ - id        │           │ - id        │           │ - id        │
│ - name      │           │ - building_id│          │ - unit_id   │
│ - address   │           │ - unit_number│          │ - email     │
│ - contact   │           │ - floor     │           │ - rut       │
└─────────────┘           └─────────────┘           └─────────────┘
       │1:N                                                │1:N
       ▼                                                   ▼
┌─────────────┐    1:N    ┌─────────────┐    1:1    ┌─────────────┐
│visitor_spots│◄──────────│availability │◄──────────│   bookings  │
│             │           │   _blocks   │           │             │
│ - id        │           │             │           │ - id        │
│ - building_id│          │ - id        │           │ - resident_id│
│ - spot_number│          │ - spot_id   │           │ - block_id  │
...
## 3.5 Endpoints Mínimos con Ejemplos
...
// Request
{
  ...
  "specialInstructions": "Vehículo SUV grande",
...
// Response 200
{
  ...
  "data": {
    ...
    "refundAmountClp": 4050, // Menos fee de cancelación
    "estimatedRefundDays": "3-5 días hábiles"
  }
}
```

**Headers:** `Authorization: Bearer jwt-token`  
**Idempotency:** Header `Idempotency-Key` para operaciones críticas  
...
// Response 200
{
  ...
  "data": {
    ...
    "processingTimeEstimate": "3-5 días hábiles"
  }
}
```
...
**Rate Limit:** Webhooks sin límite, otros endpoints 20/minute
...
Residente                PWA              API              MercadoPago    Conserjería
    ...
    │                    │ ◄───────────────│                    │             │
    │ 10. Selecciona spot│                │                    │             │
    │ ───────────────────►│ 11. POST /bookings/create           │             │
    │                    │ ───────────────►│                    │             │
    │                    │ 12. Booking pending                  │             │
    │                    │ ◄───────────────│                    │             │
    │                    │ 13. POST /payments/checkout          │             │
    │                    │ ───────────────►│ 14. Crear preferencia            │
    │                    │                │ ───────────────────►│             │
    │                    │                │ 15. Checkout URL    │             │
    │                    │ 16. Redirect    │ ◄───────────────────│             │
    │ 17. Pago en MP     │ ◄───────────────│                    │             │
    │ ───────────────────┼─────────────────┼────────────────────►│             │
    │                    │                │ 18. Webhook confirmación        │
    │                    │                │ ◄───────────────────│             │
    │                    │ 19. Update booking                   │             │
    │                    │ ◄───────────────│                    │             │
    │ 20. Confirmación   │                │                    │             │
    ...
    │ 24. Checkout (19:00)                 │                    │             │
    │ ───────────────────────────────────────────────────────────────────────►│
    │                    │ 25. POST /bookings/uuid/complete     │             │
    │                    │ ◄───────────────│                    │             │
    │ 26. SMS: "Gracias  │                │                    │             │
    │     por usar       │                │                    │             │
    │     Estaciónate    │                │                    │             │
...
- **Instrucciones:** "El código de acceso se enviará tras confirmar el pago"
...
## 3.7 Integración de Pagos y Conciliación
...
// Endpoint métricas para Vercel Analytics
